...
exports.findOnePerson = (req, res) => {
  ...
  db.query(`select * from PEOPLE where ID=${id}`, (err, rows, fields) => {
    if (!err)
...
exports.updatePerson = (req, res) => {
  ...
  db.query(`select * from PEOPLE where ID=${id}`, (err, rows, fields) => {
    if (!err)
      if (rows.length > 0)
        db.query(
          `update PEOPLE set  NAME='${person.name}' , FIRSTNAME='${person.fName}' , MAIL='${person.mail}' WHERE ID=${id}`,
          (err, rows, fields) => {
...
exports.updatePlayer = (req, res) => {
  ...
  db.query(`select * from PLAYER where ID =${id}`, (err, rows, fields) => {
    if (!err)
      if (rows.length > 0)
        db.query(
    ...
    (err, rows, fields) => {
      if (!err)
        if (rows.length > 0) {
          ...
          db.query(
            `set PLAYER_FK1=0; delete from PEOPLE where PEOPLE_ID =${id}; set PLAYER_FK1=1`,
            (err, rows, fields) => {
              if (!err)
                res.send({
                  message: "deleted",
...
exports.findAllPlayers = (req, res) => {
  db.query(
    `select * from PEOPLE pe inner join PLAYER pl on pl.ID=pe.ID  `,
    (err, rows, fields) => {
      if (!err) res.send(rows);
      else
        res.status(500).send({
...
exports.findOnePlayer = (req, res) => {
  const id = req.params.id;
  db.query(
    `select * from PEOPLE pe inner join PLAYER pl on pl.ID=pe.ID where pe.ID=${id}`,
    (err, rows, fields) => {
      if (!err)
        if (rows.length > 0) res.send(rows);
        else
  ...
  db.query(
    `  insert into GAME
                values ('${game.gName}', '${game.date}' ,'${game.type}' ,${game.duration},${game.pNumber} ,${game.author} ,${game.illustrator} ,
                        '${game.editor}' ,'${game.themeName}' ,'${game.catName}' ,'${game.extOfName}')`,
    (err, rows, fields) => {
      if (!err) res.status(303).send({ message: "game created" });
      else
        res.status(500).send({
          message:
...
exports.findAllGames = (req, res) => {
  db.query(
    `select GAME.ID as id, GAME.NAME as name,
                    EXTENSION_OF as ExtensionOf, 
                    DATE_FORMAT(DATE,"%a/%b/%y") as Created,
                    TYPE as Type,
                    DURATION as 'Duration (min)',
                    NUMBER as 'Max players/game',
                    A.ID as Author,
                    I.ID as Illustrator,
                    EDITOR as Editor,
                    THEME_ID as Theme,
                ...
                from GAME 
                inner join PEOPLE as A on GAME.AUTHOR = A.ID
                inner join PEOPLE as I on GAME.ILLUSTRATOR = I.ID`,
    (err, rows, fields) => {
      if (!err) res.send(rows);
      else
        res.status(500).send({
          message: err.message || "Some error occurred while finding games.",
        });
    }
  );
};
...
exports.findOneGame = (req, res) => {
  ...
  db.query(
    ...
    (err, rows, fields) => {
      if (!err)
        if (rows.length > 0) res.send(rows); // todo check if >1 ?
        else
          res.status(404).send({
  ...
  db.query(
    ...
    (err, rows, fields) => {
      if (!err)
        if (rows.length > 0)
          db.query(
            `update GAME set APPARITION_DATE='${game.date}' , GAME_TYPE='${game.type}' ,
